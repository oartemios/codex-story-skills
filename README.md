...
## Временное правило работы с main

Пока для этого приватного репозитория недоступна branch protection на текущем тарифе GitHub, действует рабочее соглашение:

- изменения в `main` не вносятся напрямую без необходимости
- основной путь изменений: branch -> PR -> зелёный CI -> merge
- если приходится пушить напрямую, сначала локально пройти `validate-skills.py` и `sync-to-codex.sh --dry-run`

Когда GitHub позволит branch protection для этого репозитория, это правило нужно заменить техническим ограничением на уровне `main`.
