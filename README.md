# codex-story-skills

![Skills CI](https://github.com/oartemios/codex-story-skills/actions/workflows/ci.yml/badge.svg)

Набор Codex skills для художественных проектов: проектной архитектуры, сюжетной диагностики и стартовой сборки рабочей базы.

## Структура репозитория

```text
codex-story-skills/
  scripts/
  CONTRIBUTING.md
  INSTALL.md
  LICENSE
  README.md
  SECURITY.md
  skills/
    CONVENTIONS.md
    _shared/
    continuity-keeper/
    project-orchestrator/
    writer-assistant/
    story-analyst/
    project-bootstrap/
```

Каталог `skills/` сохранён как корневой runtime-слой, чтобы внутренние ссылки вроде `skills/CONVENTIONS.md` и `skills/_shared/...` продолжали работать без дополнительной упаковки.

## Текущие skills

- `project-orchestrator`: маршрутизация больших задач, выбор следующего skill и порядок шагов
- `continuity-keeper`: контроль изменений канона, сломанных связей и последствий после правок
- `writer-assistant`: редакторско-архитектурная поддержка художественного проекта
- `story-analyst`: диагностика ритма, противоречий, арок и сюжетных рисков
- `project-bootstrap`: стартовая раскладка и миграционная поддержка художественного проекта

## Позиционирование

Этот репозиторий поддерживается как единая система, а не как набор полностью независимых skill-пакетов.

Skills рассчитаны в первую очередь на fiction-first workflow и сейчас опираются на рекомендуемую структуру проекта:

- `canon/`
- `characters/`
- `books/`
- `archive/`
- `inbox/`

Эта структура является соглашением, а не жестким требованием. Текущая версия лучше всего подходит для проектов, которые уже используют такую схему или могут сопоставить с ней свои материалы без тяжелой миграции.

## Локальная установка

Для безопасной синхронизации использовать штатный скрипт:

```bash
scripts/sync-to-codex.sh
```

Для предварительной проверки без изменений:

```bash
scripts/sync-to-codex.sh --dry-run
```

Скрипт:

- валидирует пакет перед синхронизацией
- создает backup управляемых элементов
- аккуратно заменяет старые симлинки, если они остались
- синхронизирует только runtime-слой `skills/`

После установки нужно перезапустить Codex, чтобы он перечитал доступные skills.

## Установка для конечного пользователя

Для установки из GitHub и безопасной синхронизации в `~/.codex/skills`:

```bash
scripts/install-package.sh
```

Скрипт:

- клонирует пакет
- валидирует его
- устанавливает в Codex Home через безопасный sync
- показывает краткий обзор возможностей, примеры запросов и ссылки на документацию

## Разработка и контрибьютинг

Быстрый цикл разработки:

```bash
python3 scripts/validate-skills.py
scripts/sync-to-codex.sh --dry-run
scripts/sync-to-codex.sh
```

Подробные правила разработки и локального тестирования:

- `CONTRIBUTING.md`

## Smoke-test после обновления

После `scripts/sync-to-codex.sh` и перезапуска Codex полезно быстро проверить пакет на реальных запросах:

- `Хочу проработать книгу 1`
- `Что мне делать сначала, а что потом`
- `Что изменилось после этих правок`
- `Собери рабочий канон`
- `Проверь ритм книги`
- `Собери минимальный каркас проекта`

Если пакет работает корректно:

- orchestration-задачи уводят в `project-orchestrator`
- задачи про изменения версий уводят в `continuity-keeper`
- канон и структура уходят в `writer-assistant`
- диагностика уходит в `story-analyst`
- стартовая сборка и миграция уходит в `project-bootstrap`

## GitHub Actions

После каждого `push` в `main` и на каждом `pull_request` запускается CI в GitHub Actions.

CI проверяет:

- валидацию пакета
- корректность YAML-шапок и внутренних markdown-ссылок
- синтаксис shell-скриптов
- dry-run безопасной синхронизации
- реальную синхронизацию в изолированный fake Codex Home

## Правило работы с main

Для публичного репозитория `main` считается стабильной веткой пакета.

- основной путь изменений: branch -> PR -> зелёный CI -> merge
- прямой push в `main` допустим только для небольших maintainer-правок после локальной проверки
- перед merge или прямым push нужно пройти `validate-skills.py` и `sync-to-codex.sh --dry-run`
- branch protection или ruleset желательно включить на уровне GitHub, если репозиторий начинает принимать внешние PR

## Лицензия

Код и документация распространяются по лицензии MIT:

- `LICENSE`

## Языковая политика

- Основной язык репозитория: русский.
- Английский допустим для технических идентификаторов, названий файлов, путей и устойчивых терминов интерфейса.
- Если термин не является техническим идентификатором, предпочтителен русский вариант.

## Публичный статус

Репозиторий опубликован как открытый пакет Codex skills.

Текущие ограничения:

- skills всё ещё заметно подстроены под fiction-first workflow
- рекомендуемая структура проекта является соглашением, а не универсальным стандартом
- naming и маршруты могут уточняться по мере появления внешних сценариев использования
